...
- [설계 원칙 — 6-Layer Framework](PRINCIPLES.md)
...
> 전체 흐름을 먼저 조감하고 싶다면 [EDA Overview](EDA-OVERVIEW.md)를,
...
Step 7까지 완주하면, 이 전체를 관통하는 설계 원칙을 6개 레이어로 정리한 [PRINCIPLES.md](PRINCIPLES.md)로 복습할 수 있습니다.
내구성 → 원자성 → 전달 보장 → 확인 → 멱등성 → 관측. 아래 레이어가 무너지면 위 레이어는 의미가 없습니다.
...
이 구분은 Step 3(DB 저장 대상), Step 6(토픽 설계)에서 계속 돌아옵니다.
...
- `@EnableAsync` 없이 `@Async`를 달면 조용히 동기 실행된다
...
At Least Once 환경에서 중복 소비를 방어하는 멱등 패턴을 구현합니다.
**컨슈머는 프로듀서를 신뢰하면 안 된다.** 이벤트를 의심하고 검증해야 한다.
