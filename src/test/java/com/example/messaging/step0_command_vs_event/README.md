...
단, 이 기준은 **현재 아키텍처** 기준이다. 쿠폰 서비스가 별도 DB로 분리되면, 지금은 동기로 처리하는 쿠폰 사용도 하나의 TX로 묶을 수 없게 된다. 그때는 이벤트 기반 + 상태 전이(PENDING → CONFIRMED)로 설계가 바뀔 수 있다. 이 판단은 고정된 것이 아니라, 아키텍처가 바뀌면 같이 바뀐다.
