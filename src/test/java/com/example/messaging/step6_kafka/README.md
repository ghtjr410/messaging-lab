...
```
질문: "이 이벤트가 유실되면 어떤 일이 생기는가?"

답에 따라:
  "돈이 안 맞는다" / "주문이 사라진다" / "결제가 꼬인다"
  → Outbox 필수. 복잡도를 감수한다.

  "좋아요 수가 1개 틀릴 수 있다" / "알림이 안 갈 수 있다"
  → Outbox 없이 직접 발행해도 된다. 유실 확률은 극히 낮다.
```

| 이벤트 유형 | 유실 비용 | 전략 | 이유 |
|-----------|----------|------|------|
| 결제 완료, 주문 생성, 정산 | **매우 높음** (금전 불일치) | Outbox 필수 | 유실 = 즉시 사업 손실 |
| 재고 차감, 쿠폰 발급 | **높음** (데이터 정합성) | Outbox 권장 | 유실 = 초과 판매/초과 발급 |
| 좋아요, 조회수, 추천 집계 | **낮음** (통계 오차) | 직접 발행 | 1건 유실돼도 UX 영향 미미 |
| 로그 수집, 분석 이벤트 | **매우 낮음** | 직접 발행 (acks=0도 가능) | 유실보다 처리량이 중요 |

> 좋아요/조회수가 "분석 집계용"이 아니라 "API 응답의 유일한 소스"라면 유실 비용은 "높음"으로 올라간다. 별도 실시간 카운터가 있는지 여부로 판단이 달라진다.

### Outbox 없이 직접 발행하는 구조

유실 비용이 낮은 이벤트는 `@TransactionalEventListener(AFTER_COMMIT)` + 직접 Kafka 발행으로 충분하다.
...
단, 이건 규모에 따라 달라진다. 좋아요가 초당 수만 건이면 위의 복잡도가 실제 문제가 되고, Outbox 선별 적용이 맞다. 초당 수십~수백 건이면 "패턴 2개를 유지하는 인지 비용"이 "Outbox INSERT 비용"보다 클 수 있다. 그때는 전부 Outbox로 통일하는 것도 합리적이다. 핵심은 "Outbox를 쓸까 말까"가 아니라 **"유실 비용 vs 복잡도 비용"을 따지는 것**이다.
